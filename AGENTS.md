...
- 拡張機能のメタデータ（名前・説明など）は `lib/github-enhancer-metadata.ts` に集約してください。
- popup/background 間の message type は `lib/github-enhancer-protocol.ts` に集約してください。呼び出し側へ生文字列を増やさないでください。
